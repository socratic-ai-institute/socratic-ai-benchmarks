...
class SocraticBenchmarksDB:
    ...
    def __init__(self, table_name: str = 'SocraticBenchmarks', region: str = 'us-east-1'):
    ...
    def convert_floats_to_decimal(obj: Any) -> Any:
        ...
        if isinstance(obj, float):
            ...
            return {k: SocraticBenchmarksDB.convert_floats_to_decimal(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            return [SocraticBenchmarksDB.convert_floats_to_decimal(item) for item in obj]
        return obj
    ...
    def convert_decimal_to_float(obj: Any) -> Any:
        ...
        if isinstance(obj, Decimal):
            ...
            return {k: SocraticBenchmarksDB.convert_decimal_to_float(v) for k, v in obj.items()}
        elif isinstance(obj, list):
    ...
    def get_session(self, session_id: str) -> Optional[Dict]:
        ...
        session_data['interventions'].sort(key=lambda x: x['intervention_metadata']['timestamp'])
    ...
    def record_dynamic_intervention(
        ...
        avg_question_quality = sum(
            q['quality_scores'].get('question_relevance', 0)
            for q in socratic_sequence
        ) / len(socratic_sequence)

        avg_answer_depth = sum(
            q['quality_scores'].get('answer_depth', 0)
            for q in socratic_sequence
        ) / len(socratic_sequence)
        ...
        self.update_session_status(session_id, {
            'interventions_completed': sequence_number
        })
    ...
    def record_assessment(
        ...
        # Update session status
        status_key = f'{assessment_type}_completed' if assessment_type != 'midpoint' else 'baseline_completed'
    ...
    def get_sessions_by_condition(
        ...
        return [self.convert_decimal_to_float(item) for item in response.get('Items', [])]
    ...
    def get_interventions_by_type(self, intervention_type: InterventionType) -> List[Dict]:
        """Query all interventions of a specific type"""
        response = self.table.query(
            IndexName='GSI4',
            KeyConditionExpression='GSI4PK = :type',
            ExpressionAttributeValues={
                ':type': f'INTERVENTION#{intervention_type}'
            }
        )
