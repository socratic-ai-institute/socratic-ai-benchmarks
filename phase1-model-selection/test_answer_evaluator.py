...
from socratic_eval.context_growth.socratic_answer_evaluator import (
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        try:
            score = evaluate_socratic_answer(
                response=test["response"], token_count=test["token_count"]
            )
            ...
            print("\nValidation:")
            print(
                f"  Expected Socratic Ending: {test['expected']['ends_with_socratic_question']}"
            )
            print(f"  Actual:                   {score['ends_with_socratic_question']}")
            print(
                f"  ✓ PASS"
                if score["ends_with_socratic_question"]
                == test["expected"]["ends_with_socratic_question"]
                else "  ✗ FAIL"
            )
