...
{{"open_ended": 0.0, "probing": 0.0, "builds_on_previous": 0.0, "age_appropriate": 0.0, "content_relevant": 0.0}}"""
    ...
    try:
        ...
        result = call_bedrock_model(
            "anthropic.claude-3-5-sonnet-20241022-v2:0", "anthropic", judge_prompt
        )
...
def test_model(model: Dict, scenarios: List[Dict], progress_callback=None) -> Dict:
    ...
    for i, scenario in enumerate(scenarios):
        ...
        if generation["error"]:
            errors += 1
            results.append(
                {"scenario_id": scenario["id"], "error": generation["error"]}
            )
            print(
                f"   ❌ Scenario {i + 1}/{len(scenarios)}: ERROR - {generation['error'][:50]}"
            )
            continue
        ...
        if (i + 1) % 10 == 0:
            avg_quality = sum(
                ...
                for r in results
                if "quality_scores" in r
            ) / len([r for r in results if "quality_scores" in r])
            print(
                f"   ✓ Progress: {i + 1}/{len(scenarios)} | Avg Quality: {avg_quality:.3f}"
            )
    ...
    if successful_results:
        avg_quality = sum(
            r["quality_scores"]["overall"] for r in successful_results
        ...
        criteria_scores = {
            c: sum(r["quality_scores"][c] for r in successful_results)
            / len(successful_results)
            for c in criteria
        }
    else:
    ...
    print(
        f"   ✅ Complete: {summary['successful']}/{len(scenarios)} successful | Avg Quality: {avg_quality:.3f} | Avg Latency: {avg_latency:.0f}ms"
    )
...
def print_summary(results: List[Dict]):
    ...
    for i, r in enumerate(sorted_results):
        ...
        print(
            f"{rank_display:<6} {r['model_name']:<35} {quality_str:<10} {latency_str:<12} {success_str:<10}"
        )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick test (2 models, 10 scenarios)"
    )
    parser.add_argument(
        "--models", type=int, help="Number of models to test (default: all 8)"
    )
    parser.add_argument(
        "--scenarios", type=int, help="Number of scenarios to use (default: 120)"
    )
