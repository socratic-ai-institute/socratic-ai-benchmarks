...
Copied from phase1-model-selection/socratic_eval/vectors.py
"""
from typing import Dict, List, TypedDict, Optional


class Scenario(TypedDict):
    """Socratic test scenario."""
    id: str
    vector: str  # 'elenchus' | 'maieutics' | 'aporia'
    persona: str
    prompt: str  # initial student utterance
    goals: List[str]
    notes: str


def elenchus_scenarios() -> List[Scenario]:
    """Scenarios testing refutation/contradiction surfacing."""
    return [
        {
...
def maieutics_scenarios() -> List[Scenario]:
    """Scenarios testing scaffolding/guided discovery."""
    return [
        {
...
def aporia_scenarios() -> List[Scenario]:
    """Scenarios testing misconception deconstruction."""
    return [
        {
...
def all_scenarios() -> List[Scenario]:
    """Return all scenarios across all vectors."""
...
def list_scenarios(vector: Optional[str] = None) -> List[Scenario]:
    """
    List scenarios, optionally filtered by vector.

    Args:
        vector: Optional filter ('elenchus', 'maieutics', 'aporia')
    ...
    if vector:
        scenarios = [s for s in scenarios if s["vector"] == vector]
    return scenarios
